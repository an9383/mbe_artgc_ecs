...
namespace KR.MBE.CommonLibrary
{
    ...
    public class Util
    {
        ...
        public static void FlattenJson(JObject jObject, List<Dictionary<string, string>> listdata)
        {
            ...
            foreach (var property in jObject.Properties())
            {
                if (property.Value is JObject nestedObject)
                {
                    ...
                    FlattenJson(nestedObject, listdata);
                }
                else
                {
                    // 기본 값인 경우 Dictionary에 추가
                    dict[property.Name] = property.Value.ToString();
                }
            }

            // Dictionary가 비어있지 않으면 리스트에 추가
            if (dict.Count > 0)
            {
                listdata.Add(dict);
            }
        ...
        {
            if( System.Windows.Forms.Control.IsKeyLocked( System.Windows.Forms.Keys.CapsLock ) == false )
            {
                const int KEYEVENTF_EXTENDEDKEY = 0x1;
                const int KEYEVENTF_KEYUP = 0x2;
