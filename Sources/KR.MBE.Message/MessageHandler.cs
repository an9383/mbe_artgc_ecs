...
namespace KR.MBE.Message
{
    ...
    public class MessageHandler
    {
        ...
        */
        ...
        {
            return m_oSender_ActiveMQ.getConfigData();
        }
        ...
        private static void ReadConfigFile()
        {
            if( m_Middleware == MiddlewareType.NONE )
            {
                ...
                if( sMiddlewareType.ToUpper() == "WEBLOGIC" )
                {
                    m_Middleware = MiddlewareType.WEBLOGIC;
                }
                if( sMiddlewareType.ToUpper() == "TIBCO" )
                {
                    m_Middleware = MiddlewareType.TIBCO;
                }
                if( sMiddlewareType.ToUpper() == "JMS" )
                {
                    m_Middleware = MiddlewareType.JMS;
                }
        ...
        private static DataSet sendMessage_WEBLOGIC( string strMessageName, Hashtable htBody, string strDataListTag, string strDataID, string strDataInfoTag, ArrayList arrBindValue )
        {
            ...
            {
                MBE.CommonLibrary.LogManager.ErrorLog( "make XML Header Message. Please Check the middleware config file" );
                return dsReturn;
            }
        ...
        */
        ...
        private static DataSet sendMessage_ACTIVEMQ( string strMessageName, Hashtable htBody, string strDataListTag, string strDataID, string strDataInfoTag, ArrayList arrBindValue )
        {
            ...
            using (Util.BeginBusyBlock())
            {
                ...
                {
                    //MBE.CommonLibrary.LogManager.ErrorLog("make XML Header Message. Please Check the middleware config file");
                    Log.ErrorLog("make XML Header Message. Please Check the middleware config file");
                    return dsReturn;
                }
        ...
        /// <param name="strReplySubject">Reply Subject : PROD.KR.MBE.UI-192.168.0.12.TEST</param>
        /// <param name="strSourceSubject">Source Subject : PROD.KR.MBE.UI-192.168.0.12.TEST</param>
        /// <param name="strTargetSubject">Target Subject : PROD.KR.MBE.TESTsvr</param>
        /// <returns></returns>
        ...
        public static string GetXMLFromHashtable(Hashtable data)
        {
            ...
            if (data.Count > 0)
            {
                ...
                while (list.MoveNext())
                {
                    strData += addXMLTag(list.Key.ToString(), list.Value.ToString());
                }
            }
        ...
        public static string GetSendMessageFormat(Hashtable htHeader, Hashtable htBody, Hashtable htReturn = null)
        {

            string strHeader = addParentXMLTag("header", GetXMLFromHashtable(htHeader));
            string strBody = addParentXMLTag("body", GetXMLFromHashtable(htBody));

            if (htReturn != null)
            {
                strBody += addParentXMLTag("return", GetXMLFromHashtable(htReturn));
            }
        ...
        public static void SendMessageAsync(Hashtable htHeader, Hashtable htBody, Hashtable htReturn = null, string replyTo = "", string replyId = "")
        {
            try
            {
                string strSendMessage = GetSendMessageFormat(htHeader, htBody, htReturn);
                ...
                if (m_Middleware == MiddlewareType.ACTIVEMQ)
                {
                    Log.InfoLog($"{htHeader["messagename"].ToString()} ######################");
                    Log.InfoLog(strSendMessage);
                    Log.InfoLog($"{htHeader["messagename"].ToString()} ######################");
                    m_oSender_ActiveMQ.SendTopic(strSendMessage, replyTo, replyId);
                }
            }
            catch (Exception ex)
            {
        ...
        public static void SendMessageAsyncEco(string strTransactionID, Hashtable htBody)
        {
            string strHeader = string.Empty;
