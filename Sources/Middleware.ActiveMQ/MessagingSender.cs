...
namespace Middleware.ActiveMQ
{
    ...
    public class Sender
    {
        ...
        private string m_sEcsTopicName = "jmsTopic";
        ...
        public void SendTopic(String sMessage, string sReplyTo = "", string sReplyId = "")
        {
            ...
            try
            {
                ...
                using (ISession session = connection.CreateSession())
                {
                    ...
                    // 초기값은 동일한 TopicName 이며
                    // Active MQ 의 Virtual Topic 을 사용하는 경우 QueueName 을 설정해 주어야 한다.
                    IDestination topicDestination;
                    IDestination queueDestination;
                    if (string.IsNullOrEmpty(sReplyTo))
                    { 
                        topicDestination = session.GetTopic(this.m_sEisTopicName);
                        queueDestination = session.GetTopic(this.m_sEisTopicName);
                    }
                    else
                    {
                        if (sReplyTo.Contains('/'))
                        {
                            int pos = sReplyTo.LastIndexOf('/');
                            sReplyTo = sReplyTo.Substring(pos + 1);
                        }

                        topicDestination = session.GetTopic(sReplyTo);
                        queueDestination = session.GetTopic(sReplyTo);
                    }
                    if (this.m_sQueueName != null && this.m_sQueueName != "-" && this.m_sQueueName == "")
                    {
                        queueDestination = session.GetQueue(this.m_sQueueName);
                    ...
                    string sTransactionID = Middleware.ActiveMQ.Util.GetXMLRecord(sMessage, "transactionid");
                    string sReplySubject = Middleware.ActiveMQ.Util.GetXMLRecord(sMessage, "replysubject");
                    string sTargetSubject = Middleware.ActiveMQ.Util.GetXMLRecord(sMessage, "targetsubject");
                    ...
                    using (IMessageProducer producer = session.CreateProducer(topicDestination))
                    {
                        ...
                        if (string.IsNullOrEmpty(sReplyId) == false)
                        {
                            request.NMSCorrelationID = this.m_sClientID;
                        }
                        else
                        {
                            request.NMSCorrelationID = sReplyId;
                        }
                        ...
                        request.Properties["transactionid"] = sTransactionID;
                        request.Properties["subject"] = string.IsNullOrEmpty(sTargetSubject) ? m_sTargetSubject : sTargetSubject;
                        request.Properties["replysubject"] = sReplySubject;
